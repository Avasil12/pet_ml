...
@user_route.get("/private2", response_class=HTMLResponse)
...
@user_route.get("/auth/register", response_class=HTMLResponse)
...
@user_route.post("/auth/register", response_class=HTMLResponse)
...
@user_route.get("/all", response_model=List[UserRead])
def get_all_users(db: Session = Depends(get_session)) -> List[UserRead]:
    users = UsersService.get_all_users(db)
    return users


@user_route.get("/auth/login", response_class=HTMLResponse)
...
@user_route.post("/auth/login", response_class=HTMLResponse)
...
@user_route.get("/auth/logout", response_class=HTMLResponse)
async def login_get():
